...
def _pick_config_dir(root: Path) -> Path:
    """
    Repo layout v2 prefers `configs/`. Keep a fallback to legacy `config/` for a transition period.
    """
    try:
        p = root / "configs"
        if p.exists():
...
def create_app(config_path: Path) -> Flask:
    ...
    defaults_path = _pick_config_dir(repo_root) / "defaults.yaml"
    cfg = None
...
def main():
    ...
    if "--run-pipeline" in sys.argv:
        ...
        scripts_dir = (root / "pipelines") if (root / "pipelines").exists() else (root / "scripts")
