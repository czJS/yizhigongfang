...
# We DO NOT list pipeline scripts as entry scripts in Analysis (it would execute them sequentially and
...
# Shared lightweight utilities (paths/config shims, etc.)
try:
...
# Project data files used by zh phrase extraction (idiom dict).
# Keep paths stable across dev / packaged onefile by placing them under assets/zh_phrase/.
try:
    _p = _ROOT / "assets" / "zh_phrase" / "idioms_4char.txt"
    if _p.exists():
        _DATAS.append((str(_p), str(Path("assets") / "zh_phrase")))
except Exception:
    pass
try:
    _p = _ROOT / "assets" / "zh_phrase" / "idioms_extra.txt"
    if _p.exists():
        _DATAS.append((str(_p), str(Path("assets") / "zh_phrase")))
except Exception:
    pass
...
a = Analysis(
    [str(_ROOT / "apps" / "worker_quality" / "quality_worker_entry.py")],
    # Ensure the backend package can be resolved from apps/backend/backend (no root-level backend/ shim).
