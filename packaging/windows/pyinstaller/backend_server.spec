...
_SCRIPT_TARGETS = [
    str(_ROOT / "pipelines" / "lite_pipeline.py"),
    str(_ROOT / "pipelines" / "online_pipeline.py"),
]

a = Analysis(
    [str(_ROOT / "apps" / "backend" / "backend" / "app.py"), *_SCRIPT_TARGETS],
    ...
    binaries=[],
    datas=[],
    # Keep shared lightweight utilities bundled (used by backend/runtime_paths and others).
    hiddenimports=collect_submodules("pipelines") + collect_submodules("core") + collect_submodules("backend"),
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
