...
$specCandidates = @(
  (Join-Path $repo "packaging\pyinstaller\quality_worker.spec"),
  (Join-Path $repo "quality_worker.spec")
)
$spec = $null
foreach ($p in $specCandidates) {
  if (Test-Path $p) { $spec = $p; break }
}
if (-not $spec) { throw "[quality_worker] spec not found under packaging/pyinstaller or repo root." }
