...
if (-not (Test-Path $desktopDir)) {
  # Transition fallback for older layout.
  $desktopDir = Join-Path $repoRoot "frontend"
}
...
if (-not (Test-Path $backendExe)) {
  throw "[pack] missing dist/backend_server.exe. Please run packaging/scripts/windows/rebuild_backend_server_exe.ps1 first."
}
if (-not (Test-Path $qualityWorkerExe)) {
  throw "[pack] missing dist/quality_worker.exe. Please run packaging/scripts/windows/rebuild_quality_worker_exe.ps1 first."
}
...
# Use packaging/scripts/windows/build_ollama_pack.ps1 to produce a separate ollama_pack.zip for user import.
...
if (-not $SkipDist) {
  ...
  try {
    ...
    if ($needsDeps) {
      # First-time setup on a new machine / after moving frontend -> apps/desktop
      $legacyNm = Join-Path $repoRoot "frontend\\node_modules"
      if (Test-Path $legacyNm) {
        Write-Host "[pack] node_modules missing/incomplete; syncing from legacy frontend/node_modules to avoid network..."
        robocopy $legacyNm $nm /MIR /SL /NFL /NDL /NJH /NJS /NP | Out-Null
        $rc = $LASTEXITCODE
        if ($rc -ge 8) { throw "[pack] robocopy node_modules failed (exit $rc)" }
      } else {
