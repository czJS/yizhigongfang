...
def e2e_score_from_quality_report(rep: Dict[str, Any]) -> Dict[str, Any]:
    ...
    trunc_fail = False
    # If generate_quality_report marked passed false due to truncation, it will be in errors;
    # still explicitly check truncation numbers for penalty scaling.
    ...
    if isinstance(trunc_s, (int, float)) and isinstance(trunc_ratio, (int, float)) and trunc_s > 0:
        # treat meaningful truncation as hard-ish
    ...
    # Start from 100 then subtract penalties. This makes "shipping failures" dominate.
    score = 100.0
    if missing > 0:
        score -= 50.0
    ...
    # Soft penalties (cap to avoid runaway)
