...
$logsDir = Join-Path $repo "scripts"
...
$existingPack = Join-Path $repo "frontend\dist_electron\models_pack"
...
Write-Host ("  installer: {0}" -f (Join-Path $repo "frontend\dist_electron\YizhiStudio-0.1.0-win.exe"))
Write-Host ("  modelpack:  {0}" -f (Join-Path $repo "frontend\dist_electron\models_pack.zip"))
