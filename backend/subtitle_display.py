...
def _best_split_point(text: str) -> int:
    ...
    if not t:
        return -1
    mid = len(t) // 2
    # Prefer sentence/phrase boundary punctuation near the middle
...
def build_display_items(
    *,
    ...
    # 1) Merge adjacent items (conservative)
    ...
    if merge_enable and items:
        ...
        for nxt in items[1:]:
            ...
            should_merge = (
                ...
                and (len(cur_txt) <= 22 or len(nxt_txt) <= 22)  # only merge when at least one side is short
                and not re.search(r"[.!?]$", cur_txt)  # avoid merging after sentence end (keeps rhythm)
            )
            if should_merge:
                cur = DisplayItem(
    ...
    # 2) Split long items (deterministic)
    ...
    for it in merged:
        ...
        # allocate time by character weight (rough)
        wa = max(len(a), 1)
        wb = max(len(b), 1)
