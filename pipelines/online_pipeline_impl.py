...
from pipelines.lib.asr.lite_asr import enforce_min_duration, extract_audio, write_srt
from pipelines.lib.tts.lite_tts import AudioSegment, clean_tts_text, save_audio, split_for_tts, stretch_or_pad
...
def main() -> None:
    ...
    for idx, seg in enumerate(seg_en, 1):
        ...
        seg.translation = clean_tts_text(seg.translation)
        parts = split_for_tts(seg.translation, max_len=args.tts_split_len)
        ...
        for j, part in enumerate(parts):
            ...
            if AudioSegment is None:
                raise RuntimeError("pydub is required for online pipeline TTS stitching.")
            wav = AudioSegment.from_file(seg_wav)
            wav_aligned = stretch_or_pad(wav, target_ms=part_ms, allow_speed_change=True, max_speed=args.tts_speed_max)
        ...
        combined_part = stretch_or_pad(combined_part, target_ms=target_ms, allow_speed_change=True, max_speed=args.tts_speed_max)
    ...
    save_audio(combined, tts_wav, sample_rate=args.sample_rate)
