...
def ref_free_readability_score(text: str) -> float:
    ...
    Signals (heuristic):
    - reasonable length (tokens) for a subtitle line
    - not too many non-ascii chars
    - has end punctuation sometimes (soft preference)
    - low repetition
    """
    t = _norm_en(text)
    toks = _tokens(t)
...
def quality_score(bleu: float, chrf: float, len_ratio: float, rep3: float) -> float:
    ...
    - base quality: 0.75*chrF + 0.25*BLEU (both 0..1)
    - penalize abnormal length drift from reference (len_ratio away from 1.0)
    - penalize repetition (rep3)

    This is NOT a universal metric; it's a stable scalar for regression tracking.
    """
    base = 0.75 * float(chrf) + 0.25 * float(bleu)
    # length penalty: allow small drift; penalize more as it moves away from 1
    lr = float(len_ratio)
    len_pen = max(0.0, 1.0 - 0.25 * abs(lr - 1.0))  # 1.0 drift -> 0.75
    # repetition penalty: rep3 is already 0..1-ish; keep it mild
...
def main() -> None:
    ...
    # remove heavy per-item lists before saving
    ...
    report = {
        "说明_cn": {
            "用途": "英文自然度/口语字幕评测（public-only 或 golden）。用于横向对比不同增强开关，并给出绝对质量与置信度。",
            "主指标_primary_metric": "用于计算置信度 p_improve 的指标（默认 chrF）。p_improve 表示 bootstrap 抽样下，候选方案优于 baseline 的概率。",
            "名词解释": {
                "BLEU": "基于词 n-gram 的匹配分数（0~1），越大越接近参考译文；对同义改写比较敏感。",
                "chrF": "基于字符 n-gram 的 F-score（0~1），对词形变化更鲁棒一些，但仍会惩罚与参考差异。",
                "len_ratio": "候选英文 token 数 / 参考英文 token 数，越接近 1 越好（过短/过长都会扣分）。",
                "rep3": "3-gram 重复率近似值（0~1），越大表示重复越多。",
                "quality_score": "综合分（0~1）：0.75*chrF + 0.25*BLEU，再乘长度/重复惩罚，用于长期回归追踪。",
                "quality_score_100": "quality_score 的 100 分制（0~100），更直观。",
                "ref_free_score": "reference-free 自然度/可读性启发式分（0~1，不依赖参考译文，用于更贴近业务阅读）。",
                "ref_free_score_100": "ref_free_score 的 100 分制（0~100）。",
                "final_score_100": "最终 100 分制：以 baseline=70 为锚点，将（0.6*quality_score + 0.4*ref_free_score）映射到 0~100，便于业务阅读与回归对比。",
                "p_improve": "置信度（0~1）：bootstrap 下 Δ(primary_metric)>0 的比例。",
                "delta_mean": "bootstrap 下 Δ(primary_metric) 的平均值。",
                "ci95": "Δ(primary_metric) 的 95% 置信区间。",
            },
        },
    ...
    # final score: anchor baseline to 70 for business readability
    base_mix = 0.6 * float(report["baseline_summary"].get("quality_score", 0.0)) + 0.4 * float(report["baseline_summary"].get("ref_free_score", 0.0))
    def _final_100(mix: float) -> float:
        # map baseline->70, and scale the remaining headroom to 100
        denom = max(1e-6, 1.0 - base_mix)
        val = 70.0 + 30.0 * ((mix - base_mix) / denom)
        return float(max(0.0, min(100.0, val)))

    report["baseline_summary"]["final_score_100"] = round(_final_100(base_mix), 2)
    for k, v in report["runs"].items():
        mix = 0.6 * float(v.get("quality_score", 0.0)) + 0.4 * float(v.get("ref_free_score", 0.0))
        v["final_score_100"] = round(_final_100(mix), 2)
