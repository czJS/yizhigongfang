...
def mux_video_audio(
    ...
    *,
    ...
    def _build_erase_filter() -> tuple[str, bool]:
        ...
        # Wide bottom subtitle bands are better covered with a solid fill than blur:
        # blur leaves bright glyph ghosts on black bars, while a filled black band
        # matches the source appearance and gives the new subtitles a clean base.
        try:
            band_like = False
            wh3 = ffprobe_display_wh(video_path)
            if wh3:
                W3, H3 = wh3
                band_like = (
                    W3 > 0
                    and H3 > 0
                    and (wp / float(W3)) >= 0.75
                    and (xp / float(W3)) <= 0.12
                    and ((yp + hp) / float(H3)) >= 0.88
                    and (hp / float(H3)) >= 0.08
                )
            pad_y = max(6, min(18, int(round(hp * 0.12))))
            y_fill = max(0, yp - pad_y // 2)
            h_fill = hp + pad_y
            if wh3:
                _W3, H3 = wh3
                if y_fill + h_fill > H3:
                    h_fill = max(2, H3 - y_fill)
            fill_vf = f"drawbox=x={xp}:y={y_fill}:w={wp}:h={h_fill}:color=black@1:t=fill"
        except Exception:
            band_like = False
            fill_vf = f"drawbox=x={xp}:y={yp}:w={wp}:h={hp}:color=black@1:t=fill"

        if m in {"auto", "智能", "recommended"}:
            if band_like:
                return fill_vf, False
            m = "blur"

        if m in {"fill", "cover", "black", "black_fill", "solid"}:
            return fill_vf, False

        # Use a precise blur overlay so the affected region exactly matches the rectangle.
        if m in {"blur", "boxblur"}:
        ...
        # delogo works better on natural-image subtitles without a fixed dark band.
        xp2 = int(xp - band)
        yp2 = int(yp - band)
        wp2 = int(wp + 2 * band)
        hp2 = int(hp + 2 * band)
    ...
    def _run_ffmpeg(cmd: List[str], label: str) -> subprocess.CompletedProcess:
        ...
        print(f"[mux] ffmpeg_done ({label}): elapsed_s={(time.perf_counter() - t0):.3f}")
        return proc
    ...
    print(
        f"[mux] input_probe: video_s={f'{v_dur:.3f}' if v_dur is not None else 'na'} "
        f"audio_s={f'{a_dur:.3f}' if a_dur is not None else 'na'} "
        f"threshold_s={float(threshold_s):.3f} sync_strategy={sync_strategy}"
    )
    ...
    if v_dur is not None and a_dur is not None and a_dur > v_dur + float(threshold_s):
        ...
        print(
            f"[mux] slow_path_plan: ratio={ratio:.3f} slow_ratio={slow_ratio:.3f} "
            f"tail_pad_s={tail_pad:.3f} erase_complex={erase_complex}"
        )
        if tail_pad > 0.02:
    ...
    print(f"[mux] fast_path: mode=filter erase_complex={erase_complex}")
    cmd3 = [
        "ffmpeg",
        "-y",
        "-i",
